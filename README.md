...
<sub>Nota 1: As cores para funções decorativas também são selecionadas via window.crypto.getRandomValues(), por razões de escopo de projeto. Adicionar uma implementação de geração pseudoaleatória inferior, mesmo num contexto onde isso não piora segurança, siginifica maior complexidade de código e de auditoria, com nenhum benefício prático em performance.</sub>
