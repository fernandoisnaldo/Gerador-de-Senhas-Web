...
As cores para funções decorativas também são selecionadas via window.crypto.getRandomValues(), por razões de escopo de projeto.

Não é considerado producente implementar uma baseada em Math.random para este projeto, nem mesmo para este propósito específico.

Nota 2:

A implementação deste CSPRNG pode variar de acordo com o motor de JavaScriptdo navegador.
