...
[^7]: Para quem quiser visualizar o teste unitário no console do navegador, recomenda-se testar 100000 de caracteres nos modo "ASCII", "alfanumérico", "hexadecimal", "números" ou "base64". Devido à enorme quantidade de sílabas diferentes, um teste equivalente para o modo "sílaba" requer 200 milhões de caracteres e isso impraticável na interface deste programa. O teste unitário é feito pra detectar falhas no PRNG do ambiente de execução, e uma falha do ambiente de execução NÂO É uma falha do código deste programa.
